...
class ProductView(View):
	def get(self, request):
		...
		return render(request, 'app/home.html', {'topwears':topwears, 'bottomwears':bottomwears, 'mobiles':mobiles, 'groceries': groceries,'totalitem':totalitem})
...
def groceries(request, data=None):
	...
	return render(request, 'app/grocery.html', {'groceries':groceries, 'totalitem':totalitem})

# View for Frozen and Chilled
def frozenandchilled(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
	...
	return render(request, 'app/frozenandchilled.html', {'frozenandchilled':frozenandchilled, 'totalitem':totalitem})
